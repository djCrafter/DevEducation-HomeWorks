...
namespace RandomPong
{
    ...
    class Field
    {
        ...
        public bool OnTheBorder(Point point, int pongHeight, int pongWidth) 
        {
            if (point.PointX <= 0 || point.PointY <= 0 || point.PointX + pongWidth >= HorizontalSize ||
                point.PointY + pongWidth >= VerticalSize)
                return true;
